const updateStateProperty = (state, dataObj) => {
   const updateArray = (array, action) => {
      switch (action) {
         case "ADD":
            return [...state[dataObj.label], ...dataObj.data]
         case "UPDATE":
            return [...dataObj.data]
      }
   }
   ...
   const updateMap = (map, action) => {
      switch (action) {
         case "ADD":
            return new Map([...state[dataObj.label], [...dataObj.data]])
         case "UPDATE":
            return new Map([...dataObj.data])
      }
   }
   ...
   switch (typeof dataObj.data) {
      case "object":
         switch (dataObj.data.constructor.name) {
            case "Map":
               return updateMap(dataObj.data, dataObj.action)
            case "Array":
               return updateArray(dataObj.data, dataObj.action)
